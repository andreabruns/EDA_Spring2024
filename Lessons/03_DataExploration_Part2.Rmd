...
setwd("C:/Users/andre/OneDrive/Documents/R/EDA_2024/EDA_Spring2024")
...
# Omit N/A values
ggplot(na.omit(USGS.flow.data), aes(discharge.mean.approval)) +
  geom_bar()
...
```{r, fig.height = 3, fig.width = 4}
# Plot the frequency of discharge mean values as a histogram
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# Create a mean discharge histogram, binning values by increments of 10
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# Update the histogram to use 20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Update: limit the x-axis to values from 0 to 500.
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# Adjust graph to extract patterns
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = gage.height.mean), binwidth = 0.01) +
  scale_x_continuous(limits = c(0.75,4))
...
```{r, fig.height = 3, fig.width = 4}
# Plot frequency of gage height mean, min, and max values on one graph
ggplot(USGS.flow.data) +
...
# Plot frequency of gage height means by approval status
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# Create a boxplot of gage height means, divided by approval status
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# Create boxplots of discharge mean for every increment of 1 for gage height mean
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# Create "violin" boxplot (frequency represented by width) of mean gage height for each approval group
ggplot(USGS.flow.data) +
